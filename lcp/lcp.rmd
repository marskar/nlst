...
from a radiological read of the screening LDCT, such as maximal nodule size, the
existence of any ground glass object (GGO) nodules on the LDCT, and presence of emphysema within the
patient.
...
Cancer Risk Assessment Tool plus Computed Tomography (LCRAT+CT)
[@Robbins_2019], a statistical model that assesses cancer risk based on
prescreening factors and physician-annotated LDCT features. Towards this goal,
we first prepared a re-curated version of the National Lung Screening Trial
(NLST) [@NLST_2011] dataset. To create the re-curated NLST dataset, medical
doctors or medical students, under expert supervision from University of Oxford
...
In parallel to the deep learning work described above, we predicted the risk of
lung cancer incidence within 1 year using the LCRAT+CT model [@Robbins_2019].
LCRAT+CT is a log binomial model that interacts physician-annotated LDCT image
features with log-transformed lung cancer risk predictions [@Robbins_2019]. To
obtain NLST lung cancer risk predictions, we used the Lung Cancer Risk
Assessment Tool (LCRAT) [@Katki_2016] and a competing cause of death model that
were fit to Prostate, Lung, Colorectal, and Ovarian Cancer Screening Trial
(PLCO) [@PLCO_2000] data using the survival R package [@Therneau_2000;
@Therneau_2020]. In essence, LCRAT calculates prescreening cancer risk based on
12 known cancer risk factors and LCRAT+CT updates these prescreening risks
based on information from 11 LDCT image features.

We utilized a similar approach to improve upon the LCRAT+CT model by
interacting log-transformed lung cancer risk with the maximum LCP score in
a new log binomial model, which we named LCP+LCRAT+CT. We fit both log binomial
models (LCRAT+CT and LCP+LCRAT+CT) using the parsnip R package
[@Kuhn_2020_parsnip] and the stats R package [@R_2020] generalized linear model
backend. To compare the performance of these two log binomial models, we
calculated Bayesian Information Criterion (BIC) values and conducted
a likelihood ratio test using the stats R package [@R_2020] (Table 1). The
addition of maximum LCP score improved the lung cancer predictive performance
of LCRAT+CT, according to the BIC (780 versus 825) and the likelihood ratio
test (p-value: 2.16e-13).
...
the ggplot2 R package [@Wickham_2016]. This type of plot is referred to as
a gain curve. The plot shows the sensitivity gained by shifting individuals
from biennial to annual screening. The grey line represents the ideal gain
curve, which shows the sensitivity we could achieve if screening were guided by
perfect lung cancer predictions. The gain curves of the two log binomial models
plot are summarized by the area under the receiver operator characteristic
curve (AUROC) and gain capture statistics, calculated using the yardstick
package [@Kuhn_2019_yardstick], in Table 1. Like the BIC and LRT, the AUROC
(.87 versus .82), and gain capture (0.74 versus 0.64) also indicate that the
addition of the maximum LCP score improves the ability of the LCRAT+CT model to
predict lung cancer.
