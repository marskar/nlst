...
options(digits=7)
setwd("/Users/robbinsh_mac/Dropbox/IARC/NCI/NLST/Analysis")
...
data.screen$screen.comb <- factor(data.screen$screen.comb, levels = c(2,3,4,5,6,1),
                                  labels = c("Pos","Neg-Neg","Neg-Pos","Pos-Neg","Pos-Pos","Neg"))
...
data.screen <- mutate(data.screen, pkyears.cat=ifelse(pkyears.cont>=30 & pkyears.cont<40, "[30,40)",
                      ifelse(pkyears.cont>=40 & pkyears.cont<50, "[40,50)", ifelse(pkyears.cont>=50 & pkyears.cont<999,"[50,Inf]",NA))))
data.screen$pkyears.cat <- as.factor(data.screen$pkyears.cat)
...
load("Nodule data/abn.spl.20160810.rdata")
...
data.interval.abn$post.risk.interv.overall <- fitted.values(glm.interval)
...
with(data.screen.abn.neg, CrossTable(interval, case)) # Next-screen case counts by interval
with(data.interval, CrossTable(interval, case)) # Interval case counts by interval
...
with(filter(data.interval.abn, interval %in% c(2,3)), table(interval, screen.hist))
...
### Sensitivity analyses for effect of prior screens ###
# Model using only the T0 screen
summary(glm(case ~ log1yrisk + log1yrisk:adenop.consol -1, data=dplyr::filter(data.interval.abn, interval==1), family=binomial(link='log')))
confint(glm.int.abn)
summary(glm(case ~ log1yrisk + log1yrisk:consolidation + log1yrisk:emphysema -1, data=dplyr::filter(data.screen.abn.neg, interval==1), family=binomial(link='log')))
confint(glm.screen.neg.abn)
# Include prior screen results as terms in the exponent. Calculate median (IQR) of the absolute difference
glm.screen.neg.abn.screenhist <- glm(case ~ log1yrisk + log1yrisk:screen.comb + log1yrisk:consolidation + log1yrisk:emphysema -1, data=data.screen.abn.neg, family=binomial(link='log'))
summary(glm.screen.neg.abn.screenhist)
confint(glm.screen.neg.abn.screenhist)
data.screen.abn.neg.screenhist <- filter(data.screen.abn.neg, !is.na(screen.comb))
data.screen.abn.neg.screenhist$post.risk.neg.screenhist <- fitted.values(glm.screen.neg.abn.screenhist)
with(data.screen.abn.neg.screenhist, quantile((abs(post.risk.neg.screenhist-post.risk.abn)), probs=c(0.25, 0.5, 0.75)))
...
with(filter(data.interval.abn, interval==1), CrossTable(adenopathy, consolidation))
...
neg.s.psr.q <- quantile(filter(data.screen.abn.neg, interval==1)$prescr.1yrisk, probs=c(0.25, 0.5, 0.75)) # pre-screening risk
...
# Potential risk THRESHOLDS for longer interval after negative screen. Use both T1 and T2 detection
...
num_neg <- nrow(filter(data.screen.abn.neg, interval %in% c(1,2)))        # change to look at a particular interval
num_all <- nrow(filter(data.screen.abn, interval %in% c(1,2)))        # change to look at a particular interval
num_cases <- sum(filter(data.screen.abn.neg, interval %in% c(1,2))$case)      # change to look at a particular interval
...
for (i in seq_along(thresholds)) {
  dat <- filter(data.screen.abn.neg, interval %in% c(1,2) & post.risk.abn<=thresholds[i])  # change to look at a particular interval
  n[i] <- nrow(dat)
  perc_negs[i] <- 100*nrow(dat)/num_neg   # percent of all negatives below threshold
  perc_all[i] <- 100*nrow(dat)/num_all    # percent of all individuals below threshold
  ca_N[i] <- sum(dat$case)                   # number with cancer below threshold
  perc_of_ca[i] <- 100*sum(dat$case)/num_cases  # percent of cancers falling below threshold
...
png(file="/Users/robbinsh_mac/Dropbox/IARC/NCI/NLST/Figures/negatives_threshold_both_screens.png",width=1200,height=850)
ggplot() + geom_line(data=thres.plot.all, aes(x=perc_negs, y=perc_of_ca), size=0.8) + 
  ...
  ylab("\nDetectable next-screen cancers with delayed diagnosis, %") + xlab("\nScreen-negatives with longer-than-annual interval, %") +
...
# Get stages at diagnosis for cases under different thresholds
load("/Users/robbinsh_mac/Dropbox/IARC/NCI/NLST/Analysis/nlst_stage.Rdata") # load the stages for all NLST cases sent by Li 31 Dec 2018
stages_002 <- merge(as.data.frame(c(data.screen.abn.neg %>% filter(post.risk.abn<=0.002 & case==1) %>% select(pid))), stage, by="pid", all.x=T, all.y=F) # Cases <0.2%
stages_003 <- merge(as.data.frame(c(data.screen.abn.neg %>% filter(post.risk.abn<=0.003 & case==1) %>% select(pid))), stage, by="pid", all.x=T, all.y=F)
stages_004 <- merge(as.data.frame(c(data.screen.abn.neg %>% filter(post.risk.abn<=0.004 & case==1) %>% select(pid))), stage, by="pid", all.x=T, all.y=F)
CrossTable(stages_003$stage)
17/(33-2) # omit N=2 unknown stage
