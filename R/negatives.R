...
abn <- readRDS(here("data/abnorm_lrads_merged.rds"))
data_screen_abn <-
    merge(
        data.screen,
        abn,
...
# TODO merge in Wes' T0 data here
emph <- read_csv(here('data/T0_data.csv'))
data_screen_abn_emph <-
    merge(
        data_screen_abn,
        emph,
        by = c("pid"),
...
replacevars  <- names(abnorm_lrads_merged)[!names(abnorm_lrads_merged) %in% c("pid","interval","LRcat","LRcatcol.neg","LRcatcol.pos")]
data_screen_abn_emph[replacevars][is.na(data_screen_abn_emph[replacevars])]  <- 0
# Make a variable for including observations in Lung-RADS analysis
data_screen_abn_emph$LR.include <- (data_screen_abn_emph$LRcat %in% c("1","2","3","4A","4B","4X"))
# Merge abnormalities data (person-screen level) with data.interval
data.interval.abn <- merge(data.interval, abnorm_lrads_merged, by=c("pid","interval"), all.x=T, all.y=F)
...
abnlist <- abnlist.pos <- names(abnorm_lrads_merged)[3:32]
...
data_screen_abn_emph$log.diam <- log(data_screen_abn_emph$longest.diam+1)
...
data_screen_abn_emph$prescr.1yrisk <- risk.kovalchik(0, 1, data_screen_abn_emph, LCRAT, cox.death)
data_screen_abn_emph <- mutate(data_screen_abn_emph, log1yrisk=log(prescr.1yrisk), logit1yrisk=log(prescr.1yrisk/(1-prescr.1yrisk)))
...
data_screen_abn_emph.neg <- filter(data_screen_abn_emph, screen.result==0)
data_screen_abn_emph.pos <- filter(data_screen_abn_emph, screen.result==1)
# Make a categorical variable for diameter
data_screen_abn_emph.pos <- mutate(data_screen_abn_emph.pos, diam.cat = 1*(longest.diam==0)+2*(longest.diam>0 & longest.diam<=5)+
...
data_screen_abn_emph.pos$diam.cat <- factor(data_screen_abn_emph.pos$diam.cat, levels=c(1:6),labels=c("0","4-5","6-7","8-10","11-13","14+"))
# Create a variable for any.growth that reflects a group in which growth can't be assessed (i.e. screen=T0)
data_screen_abn_emph.pos <- mutate(data_screen_abn_emph.pos, growth.3l = 
                          1*(interval==1) + 2*(interval==2 & any.growth==0) + 3*(interval==2 & any.growth==1))
data_screen_abn_emph.pos$growth.3l <- factor(data_screen_abn_emph.pos$growth.3l, levels = c(1,2,3), labels=c("NA","No","Yes"))

# Make dataset of unique individuals for descriptive table of screen-negatives
all.subj.neg <- filter(nlst.CT, pid %in% data.interval.abn$pid | pid %in% data_screen_abn_emph.neg$pid)
...
glm.screen.neg <- glm(case ~ log1yrisk -1, data=data_screen_abn_emph.neg, family=binomial(link='log'))
data_screen_abn_emph.neg$post.risk.neg.overall <- fitted.values(glm.screen.neg)
...
glm_screen_neg_abn <- glm(case ~ log1yrisk + log1yrisk:consolidation + log1yrisk:emphysema + p_emph -1, data=data_screen_abn_emph.neg, family=binomial(link='log'), na.action=na.exclude)
